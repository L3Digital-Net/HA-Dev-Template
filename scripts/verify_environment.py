...
This script checks all requirements from ha-dev-environment-requirements.md
"""
...
def main() -> int:
    ...
    # Example Integration
    print_header("Example Integration Structure")
    example_dir = project_root / "custom_components" / "example_integration"
    results["example_manifest"] = check_file_exists(
        example_dir / "manifest.json", "manifest.json"
    )
    results["example_init"] = check_file_exists(
        example_dir / "__init__.py", "__init__.py"
    )
    results["example_const"] = check_file_exists(example_dir / "const.py", "const.py")
    results["example_strings"] = check_file_exists(
        example_dir / "strings.json", "strings.json"
    )
    ...
    if failed == 0:
        ...
        print(
            ...
            f"Home Assistant integration development.{Colors.RESET}"
        )
        return 0

    print(f"\n{Colors.BOLD}{Colors.RED}✗ Some checks failed.{Colors.RESET}")
    print(
        f"{Colors.YELLOW}Please review the errors above and consult "
        f"ha-dev-environment-requirements.md{Colors.RESET}"
    )
    return 1
